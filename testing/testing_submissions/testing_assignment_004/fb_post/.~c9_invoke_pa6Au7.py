...
reactions_list=[
    ...
    {'reacted_by_id':3,'post_id':2,'comment_id':None,'reaction':'ANGRY'},
    {'reacted_by_id':4,'post_id':3,'comment_id':None,'reaction':'SAD'},
...
def data_pop():
    ...
    reaction_objs=Reaction.objects.bulk_create(reaction_bulk_list)
...
@pytest.mark.django_db
def test_total_reaction_count_with_valid_details_returning_count(data_pop):
    # Arrage
    total_count=14
    checking_key='count'
    
    # Act
    count_in_dict=get_total_reaction_count()
    key=list(count_in_dict.keys())[0]
    
    # Assert
    assert count_in_dict['count'] == total_count
    assert checking_key == key
    
    
@pytest.mark.django_db
def test_total_reaction_count_when_no_reaction_return_count_zero():
    # Arrage
    total_count=0
    checking_key='count'
...
@pytest.mark.django_db
def test_get_reaction_metrics_with_valid_details(data_pop):
    # Arrange
    post_id=1
    ...
    # Assert
    assert type(reactions_for_post) == dict_type
    assert reactions_for_post['WOW'] == actual_reactions ['WOW']
    assert reactions_for_post['LOVE'] == actual_reactions ['LOVE']
    assert reactions_for_post['SAD'] == actual_reactions ['SAD']

@pytest.mark.django_db
def test_get_reaction_metrics_when_invalid_post_id_raise_invalidpostexception(data_pop):
    # Arrange
    post_id=100
...
@pytest.mark.django_db
def test_delete_post_when_valid_details_given(data_pop):
    # Arrange
    post_id=2
    user_id=1
    is_false=False
    ...
    # Assert
    assert post_deleted == is_false
    

@pytest.mark.django_db
def test_delete_post_when_invalid_user_id_raise_invaliduserexception(data_pop):
    # Arrange
    post_id=1
    user_id=100
...
@pytest.mark.django_db
def test_delete_post_when_invalid_post_id_raise_invalidpostexception(data_pop):
    # Arrange
    post_id=100
    user_id=1
...
@pytest.mark.django_db
def test_get_post_with_more_positive_reactions(data_pop):
    # Arrange
    posts_type_is_list=list
    post_id_1=1
    post_id_4=4
    ...
    # Assert
    assert type(posts) == posts_type_is_list
    assert posts[-2] == post_id_1
    assert posts[-1] == post_id_4
    
    

#returning post ids
@pytest.mark.django_db
def test_get_posts_reacted_by_user_when_valid_details_return_post_ids_in_list(data_pop):
    # Arrange
    user_id=4
    list_type=list
    post_list=[7,8]
...
@pytest.mark.django_db
def test_get_posts_reacted_by_user_when_invalid_user_id_raise_invaliduserexception(data_pop):
    # Arrange
    user_id=100
...
@pytest.mark.django_db
def test_get_reaction_to_post_with_valid_details_return_list_of_dict_of_users_with_their_reaction(data_pop):
    # Arrange
    post_id=1
...
@pytest.mark.django_db
def test_reaction_to_post_when_invalid_post_id_raise_invalidpostexception(data_pop):
    # Arrange
    post_id=100
...
@pytest.mark.django_db
@freeze_time("2012-01-14")
def test_get_replies_for_comment_when_invalid_details_raise_invalidcommentexception(data_pop):
    # Arrange
    comment_id= 100
...
def function_used_to_compare(post,actual_post):
    for i in  range(len(post)):
        assert post[i]['comments']['replies'] == actual_post[i]['comments']['replies']
        assert post[i]['comments'] == actual_post[i]['comments']
        assert post[i]['posted_by'] == actual_post[i]['posted_by']
        assert post[i]['reactions'] == actual_post[i]['reactions']
        assert post[i]['comments_count'] == actual_post[i]['comments_count']
        assert post[i] == actual_post[i]
        
    assert post == actual_post
    return True
...
def test_get_user_posts_with_valid_details_return_list_of_post_dict(data_pop):
    ...
    # Arrange
    user_id=1
    actual_post=[{'post_id': 1,
  'posted_by': {'name': 'sulthan',
   'user_id': 1,
   'profile_pic': 'sulthan_babu/profile_pic'},
  'posted_at': '2012-01-14 00:00:00.000000',
  'post_content': 'empty',
  'reactions': {'count': 4, 'type': ['WOW', 'SAD', 'LOVE']},
